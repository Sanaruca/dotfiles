...
-- Ctrl+j/k/l e i para navegar ventanas (ijkl layout: j=izq, i=arriba, k=abajo, l=der).
-- NOTA: <C-i> es indistinguible de <Tab> en la terminal. Tab en modo normal
-- también dispara "ventana arriba". En insert/command/visual mode no se toca.
-- TODO: probablemente cambiar esto después, queremos combinaciones tipo Ctrl+k+...
-- como en VSCode en lugar de ocupar Ctrl para navegar ventanas.
...
-- <A-i> / <A-k> = mover línea(s) arriba/abajo (silent para evitar mensajes)
...
-- u = enter insert mode (replaces i)
...
-- d en modo visual borra sin guardar en el portapapeles
vim.keymap.set("x", "d", '"_d', { desc = "Delete without yanking" })

-- <BS> en modo normal: si la línea está vacía la elimina (dd), si no borra el carácter
-- anterior y entra en insert (Xi)
...
-- cc = yank en modo visual
vim.keymap.set("x", "cc", "y", { desc = "Yank" })
-- ci = yank + mover arriba (recursivo)
vim.keymap.set("x", "ci", "ygvi", { desc = "Yank and move up" })
-- ck = yank + mover abajo (recursivo)
